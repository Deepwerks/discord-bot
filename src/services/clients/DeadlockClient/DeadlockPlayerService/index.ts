...
export default class DeadlockPlayerService implements IDeadlockPlayerService {
  ...
  GetHeroStats = async (account_id: string, hero_id?: number) => {
    ...
    return hero_id ? heroStats.find((stats) => stats.hero_id === hero_id) : heroStats;
  };

  GetMatchHistory = async (account_id: string, limit: number = 50): Promise<HistoryMatch[]> => {
  ...
  GetMMRHistory = async (account_id: string, limit: number = 50): Promise<MMRHistoryRecord[]> => {
