  ...
  async Execute(interaction: ButtonInteraction) {
    try {
      ...
      try {
        ...
        const partyId = interaction.message.content.match(/Party ID: `(\d+)`/)?.[1];
        if (!partyId) {
          throw new Error("Failed to find party ID in message content");
        }

        const matchId = await useDeadlockClient.MatchService.GetMatchIdFromPartyId(partyId);
        let deadlockMatch = await useDeadlockClient.MatchService.GetMatch(matchId);
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor("Red")
              .setDescription(t("commands.match.fetch_failed") || "Failed to fetch match data"),
          ],
        });
      }
    } catch (error) {
      ...
      if (interaction.deferred) {
        await interaction.editReply({
          content: "❌ Failed to process the finish match action. Please try again later.",
        });
      } else {
